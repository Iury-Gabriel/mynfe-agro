...
export function FilaFaturamentoPage(): ReactElement {
  ...
  function handleEmitir(pedido: FilaPedido): void {
    if (!empresaId) return
    emitir.mutate(
      { empresaId, pedidoId: pedido.id },
      {
        onSuccess: (nota) => {
          toast.success(
            `DANFE do pedido ${pedido.numero}: ${FISCAL_STATUS_LABEL[nota.status]}.`,
          )
        },
